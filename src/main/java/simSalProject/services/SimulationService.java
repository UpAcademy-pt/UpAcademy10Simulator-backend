...
public class SimulationService {
	...
	public Response createSimulation(Simulation mySimulation) {
		if (SIM_B.createSimulation(mySimulation) == "Simulation created") {
		}
	...
	public Response editSimulation (long id, Simulation mySimulationtoEdit) {
		Simulation mySimulation = SIM_B.consultSimulation(id);
		
		if (mySimulation == null) {
			SIM_B.editSimulation(mySimulation);
			return Response.status(404).entity("Simulation doesn't exist").build();
		}
		return Response.ok("Simulation successfully updated").build();
	}
	...
	public Response RemoveSimulation(@PathParam("id")long idToRemove) {
		...
		if (mySimulation == null) {
			...
			return Response.ok("Simulation successfully removed").build();
		}
	}
